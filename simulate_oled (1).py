...
def simulate_device(d_ito_nm: float, d_emitter_nm: float, d_etl_nm: float,
    ...
    """
    # build geometry and simulation cell.  For 2D simulations the x dimension
    # must be finite; we choose a small pitch (1 μm) and apply Bloch periodic
    # boundary conditions via the k_point parameter.  The geometry objects are
    # still infinite in the transverse directions (mp.inf), so this does not
    # truncate the layers.
    cell, geometry = build_multilayer(d_ito_nm, d_emitter_nm, d_etl_nm,
                                      d_ag_nm, dpml=dpml, pad=pad)
    # override the x dimension of the cell to a finite width for 2D
    # propagation.  A unit width is sufficient because the structure is
    # homogeneous in x; Bloch periodicity handles oblique angles.
    cell = mp.Vector3(1.0, 0, cell.z)
    # define PML layers only along the z direction.  When using Bloch-periodic
    # boundary conditions along x, PML may not be applied in that direction.
    pml_layers = [mp.PML(direction=mp.Z, thickness=dpml)]
    ...
    for i, freq in enumerate(frequencies):
        ...
        for j, theta in enumerate(angles):
            # compute in‑plane Bloch wavevector for oblique incidence.  In a 2D
            # simulation where the structure is uniform along x and the cell
            # width is unity, the Bloch wavevector along x is kx = k0 * n_sub * sin(theta),
            # where k0 = 2π/λ in Meep units (but since c=1 and frequency=freq, k0 = 2π*freq).
            kx = 2 * np.pi * freq * n_sub * np.sin(theta)
            sim = mp.Simulation(
                cell_size=cell,
